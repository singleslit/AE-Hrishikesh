...
template <typename T> constexpr int cntSetBits(T n) {
...
template <typename T> constexpr int log2flr(T n) {
    if (n == 0) return -1;
    if constexpr (sizeof(T) <= 4) return 31 - __builtin_clz(n);
    else return 63 - __builtin_clzll(n);
}

template <typename T> constexpr int log2cl(T n) {
    return (n == 1) ? 0 : log2flr(n - 1) + 1;
}

template <typename T> constexpr T hibit(T n) { return n ? T(1) << log2flr(n) : 0; }
...
template <typename T> constexpr T toGray(T n) { return n ^ (n >> 1); }

template <typename T> constexpr T grayToBin(T g) {
    T b = 0;
    for (; g; g >>= 1) b ^= g;
    return b;
}
...
template <typename T> constexpr T revBits(T n) {
    T res = 0;
    int b = sizeof(T) * 8;
    for (int i = 0; i < b; i++) res = (res << 1) | (n & 1), n >>= 1;
    return res;
}

template <typename T> constexpr T nextSameBits(T n) {
    T c = n & -n, r = n + c;
    return r | (((n ^ r) >> 2) / c);
}

template <typename T> constexpr bool parityEven(T n) { return cntSetBits(n) % 2 == 0; }

template <typename T> constexpr void swapXor(T &a, T &b) {
    if (&a != &b) a ^= b, b ^= a, a ^= b;
}
